...
bool GameData::passedNullCheck() {
	if (!mc.getClientInstance()) return false;
	if (!mc.getGameMode()) return false;
	if (!mc.getLocalPlayer()) return false;
	if (!mc.getLocalPlayer()->getLevel()) return false;
	if (!mc.getHIDController()) return false;
	return true;
}

bool GameData::passedItemNullCheck() {
	if (!passedNullCheck()) return false;
	PlayerInventory* plrInv = mc.getLocalPlayer()->getPlayerInventory();
	Inventory* inv = plrInv->inventory;
	if (inv->getItemStack(plrInv->selectedSlot) == nullptr) return false;
	if (!inv->getItemStack(plrInv->selectedSlot)->isValid()) return false;
	return true;
}