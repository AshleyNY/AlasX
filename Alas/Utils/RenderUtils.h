...
public:
	...
	static void drawBoxCustom(const Vec3<float>& blockPos, float size1, float size2, float size3, UIColor color, UIColor lineColor, float lineWidth, bool fill, bool outline);

	static void drawCenterBox(const Vec3<float>& center, float size, UIColor color, UIColor lineColor, float lineWidth, bool fill, bool outline);
	...
	static void drawItem(ItemStack* item, Vec2<float> pos, float opacity, float scale, bool isEnchanted);
	static void drawItem(ItemStack* item, Vec2<float> pos, float opacity, float scale);
	static void drawCenteredText(const Vec2<float>& textPos, std::string textStr, MC_Color color, float textSize = 1.f, float alpha = 1.f, bool shadow = true);
	static void drawItemDurability(ItemStack* item, Vec2<float> itemPos, float scale, float opacity, int mode);
};
