...
#include <d3d11.h>
#include "../Client/HookManager/Hooks/DirectX/DirectXHook.h"
#define M_PI 3.14159265358979323846
void ImGuiUtils::setDrawList(ImDrawList* d) {
	drawlist = d;
}
void ImGuiUtils::drawAimCircle(float r,int sides , bool filled, bool rainbow, bool toMouse) {
	auto& io = ImGui::GetIO();
	...
	for (int i = 0; i < sides; ++i) {
		...
		drawList->AddLine(lastPos, nextPos, IM_COL32(0, 0, 0, 255), 4.f); // outline | §à§é§Ö§â§ä§Ñ§ß§Ú§Ö
		drawList->AddLine(lastPos, nextPos, currentColor, 2.f); // main | §Ô§Ý§Ñ§Ó§ß§í§Û
	}
}
