...
std::string String::RemoveColorCodes(std::string str) {
    ...
    return std::regex_replace(std::regex_replace(str, std::regex("¡ì."), ""), std::regex("&."), "");
}
...
std::string String::RemoveNonAscii(std::string& str) {
    // make sure to exclude symbols like ampersands and the ¡ì char here (so they dont get removed)
